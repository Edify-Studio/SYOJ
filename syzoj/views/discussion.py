...
def discussion():
    ...
    def make_url(page, other):
        return url_for("discussion") + "?" + parse.urlencode({"page": page})
...
def article(article_id):
    ...
    def make_url(page, other):
        return url_for("article", article_id=article_id) + "?" + parse.urlencode({"page": page})
...
@oj.route("/comment/<int:comment_id>/delete")
...
@oj.route("/comment/<int:comment_id>/comment", methods=["POST"])
def comment_comment(comment_id):
    user = User.get_cur_user()
    comment = Article.query.filter_by(id=comment_id).first()
    if not comment:
        abort(404)
    if not user:
        return need_login()
    ...
    if not comment_str.replace(" ", ""):
        return show_error("请输入评论",
                          next=url_for("article", article_id=comment.article_id))

    comment_comment = Comment(comment_str, article, user, comment_id)
